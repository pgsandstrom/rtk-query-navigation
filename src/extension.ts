...
class RtqHookRedirectProvider implements vscode.DefinitionProvider {
  async provideDefinition(
    ...
    try {
      ...
      const targetUri =
        target.targetUri ??
        ("targetUri" in target ? target.targetUri : (target as vscode.Location).uri)

      // Compute base name: strip "use" prefix and "Query"/"Mutation" suffix (case-insensitive for mutation)
      const baseTmp = name.replace(/^use/, "").replace(/(Query|[Mm]utation)$/, "")
...
function getCallIdentifierWord(doc: vscode.TextDocument, pos: vscode.Position) {
  ...
  const after = doc.getText(new vscode.Range(range.end, range.end.translate(0, 128)))
  let i = 0
...
function normalizeDefs(
  v: (vscode.Location | vscode.LocationLink)[] | vscode.Location | vscode.LocationLink | undefined
): vscode.LocationLink[] {
  if (!v) return []
  const arr = Array.isArray(v) ? v : [v]
  // Convert Location to LocationLink for uniform handling
  return arr.map((item) => {
    if ("targetUri" in item) return item as vscode.LocationLink
...
function firstWordOccurrence(text: string, word: string): number {
  ...
  const rx = new RegExp(`(^|[^$_0-9A-Za-z])(${escapeReg(word)})(?![$_0-9A-Za-z])`, "m")
